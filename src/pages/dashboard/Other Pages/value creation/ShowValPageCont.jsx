...
const ShowValPageCont = () => {
    ...
    const handleEdit = (valuecreationpageId) => {
      // Redirect to edit page with homepageId
      navigate(`/Dashboard/EditValuecPage/${valuecreationpageId}`);
    };
  
    const handleDelete = async (valuecreationpageId) => {
      try {
        // Adjust the endpoint to match your API, ensuring correct ID handling
        await axios.delete(`https://prickle-balanced-archaeopteryx.glitch.me/valuecreationpage/${valuecreationpageId}`);
        // Refresh data to reflect deletion
        setData(data.filter(item => item._id !== valuecreationpageId));
      } catch (error) {
