...
const EditFinCards = () => {
    ...
    const { fincardsId } = useParams(); // Retrieve the ID from the URL
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                const { data } = await axios.get(`https://prickle-balanced-archaeopteryx.glitch.me/devops/fincards/${fincardsId}`);
    ...
    const onFinish = async (values) => {
        try {
            await axios.put(`https://prickle-balanced-archaeopteryx.glitch.me/devops/fincards/${fincardsId}`, values);
            message.success('Card updated successfully');
            navigate('/dashboard/ShowFinCards'); // Redirect to the cards display page
        } catch (error) {
    ...
    return (
        <>
            <Form form={form} layout="vertical" onFinish={onFinish}>
                ...
                {['barcarddesone', 'barcarddestwo', 'barcarddesthree', 'barcarddesfour'].map((field, index) => (
                    <Form.Item
                        key={field}
                        label={`Bar Card Description ${index + 1}`}
                        name={field}
                        rules={[{ required: true, message: `Bar Card Description ${index + 1} is required` }]}
                    >
                        <Input />
                    </Form.Item>
                ))}
                ...
                {[...Array(4)].map((_, index) => (
                    <>
                        <Form.Item
                            ...
                            rules={[{ required: true, message: `Card ${index + 1} Numeric Value is required` }]}
                        >
                            <Input />
                        </Form.Item>
                        <Form.Item
                            ...
                            rules={[{ required: true, message: `Card ${index + 1} Heading is required` }]}
                        >
                            <Input />
                        </Form.Item>
                        <Form.Item
                            ...
                            rules={[{ required: true, message: `Card ${index + 1} Description is required` }]}
                        >
                            <Input.TextArea />
                        </Form.Item>
                    </>
